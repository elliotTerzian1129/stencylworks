...
class ActorEvents_24 extends ActorScript
{
	...
	override public function init()
	{
		...
		addCollisionListener(actor, function(event:Collision, list:Array<Dynamic>):Void
		{
			...
			{
				if((actor.getAnimation() == "Disactivated"))
				{
					...
					runLater(1000 * 8, function(timeTask:TimedTask):Void
					{
						...
						Engine.engine.setGameAttribute("Time Points", 0);
						actor.setAnimation("" + "Disactivated");
					}, actor);
				}
				else
				{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((Engine.engine.getGameAttribute("Time Points") == 3))
				{
					Engine.engine.setGameAttribute("Secret", 1);
					Engine.engine.setGameAttribute("Time Points", 0);
				}
			}
		});
